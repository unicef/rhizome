...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column(u'source_data_document', 'created_at',
                      self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime(2015, 6, 11, 0, 0)),
                      keep_default=False)

        db.execute('''
            UPDATE source_data_document
            SET created_at = z.mx_created
            FROM (
            	SELECT
            		sdd.id
            		,COALESCE(x.mx_created,'2000-01-01') as mx_created
            	FROM source_data_document sdd
            	LEFT JOIN
            	(
            		SELECT document_id, max(created_at) as mx_created FROM source_datapoint sd
            		GROUP BY document_id

            	)x
            	ON sdd.id = x.document_id
            )z
            WHERE z."id" = source_data_document.id
        ''')


    def backwards(self, orm):
        # Deleting field 'Document.created_at'
        db.delete_column(u'source_data_document', 'created_at')
