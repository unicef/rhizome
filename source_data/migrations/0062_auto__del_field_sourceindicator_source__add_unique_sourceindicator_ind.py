...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'SourceCampaign', fields ['source', 'campaign_string']
        db.delete_unique(u'source_data_sourcecampaign', ['source_id', 'campaign_string'])

        # Removing unique constraint on 'SourceRegion', fields ['source', 'region_string']
        db.delete_unique(u'source_data_sourceregion', ['source_id', 'region_string'])

        # Removing unique constraint on 'SourceIndicator', fields ['source', 'indicator_string']
        db.delete_unique(u'source_data_sourceindicator', ['source_id', 'indicator_string'])

        # Deleting field 'SourceIndicator.source'
        db.delete_column(u'source_data_sourceindicator', 'source_id')

        # Adding unique constraint on 'SourceIndicator', fields ['indicator_string']
        db.create_unique(u'source_data_sourceindicator', ['indicator_string'])

        # Deleting field 'SourceRegion.source'
        db.delete_column(u'source_data_sourceregion', 'source_id')

        # Adding unique constraint on 'SourceRegion', fields ['region_string']
        db.create_unique(u'source_data_sourceregion', ['region_string'])

        # Deleting field 'SourceCampaign.source'
        db.delete_column(u'source_data_sourcecampaign', 'source_id')

        # Adding unique constraint on 'SourceCampaign', fields ['campaign_string']
        db.create_unique(u'source_data_sourcecampaign', ['campaign_string'])


    def backwards(self, orm):
        # Removing unique constraint on 'SourceCampaign', fields ['campaign_string']
        db.delete_unique(u'source_data_sourcecampaign', ['campaign_string'])

        # Removing unique constraint on 'SourceRegion', fields ['region_string']
        db.delete_unique(u'source_data_sourceregion', ['region_string'])

        # Removing unique constraint on 'SourceIndicator', fields ['indicator_string']
        db.delete_unique(u'source_data_sourceindicator', ['indicator_string'])


        # User chose to not deal with backwards NULL issues for 'SourceIndicator.source'
        raise RuntimeError("Cannot reverse this migration. 'SourceIndicator.source' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'SourceIndicator.source'
        db.add_column(u'source_data_sourceindicator', 'source',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['datapoints.Source']),
                      keep_default=False)

        # Adding unique constraint on 'SourceIndicator', fields ['source', 'indicator_string']
        db.create_unique(u'source_data_sourceindicator', ['source_id', 'indicator_string'])


        # User chose to not deal with backwards NULL issues for 'SourceRegion.source'
        raise RuntimeError("Cannot reverse this migration. 'SourceRegion.source' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'SourceRegion.source'
        db.add_column(u'source_data_sourceregion', 'source',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['datapoints.Source']),
                      keep_default=False)

        # Adding unique constraint on 'SourceRegion', fields ['source', 'region_string']
        db.create_unique(u'source_data_sourceregion', ['source_id', 'region_string'])


        # User chose to not deal with backwards NULL issues for 'SourceCampaign.source'
        raise RuntimeError("Cannot reverse this migration. 'SourceCampaign.source' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'SourceCampaign.source'
        db.add_column(u'source_data_sourcecampaign', 'source',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['datapoints.Source']),
                      keep_default=False)

        # Adding unique constraint on 'SourceCampaign', fields ['source', 'campaign_string']
        db.create_unique(u'source_data_sourcecampaign', ['source_id', 'campaign_string'])
    ...
    models = {
        ...
        'source_data.activityreport': {
            ...
            'created_at': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 10, 28, 0, 0)'}),
