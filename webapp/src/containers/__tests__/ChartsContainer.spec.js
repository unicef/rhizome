...
describe.skip ('ChartsContainer', () => {
  ...
  //     expect (ChartsContainer.propTypes).to.have.all.keys('dashboard_id')
  //   })
  // })
  describe.skip ('#deleteDashboard()', () => {
    it ('exists with correct arguments', () => {
      expect (mockChartsContainer.deleteDashboard).to.exist.and.to.have.lengthOf(1)
    })
  })
  describe.skip ('#render()', () => {
    ...
    context ('if dashboard list is null', () => {
      it ('renders `Loading` message', () => {
        let actualComponent = shallow(<ChartsContainer {...{ dashboards: { raw: null } }}/>)
        actualComponent = actualComponent.debug()
  ...
  describe.skip ('events', () => {
    describe ('when onClick is clicked with `accept` response', () => {
      it.skip ('`DashboardContainerActions.deleteDashboard(id)` is called', () =>{
        ...
        const id = ChartsContainerTest.getState().dashboards.raw.id
        mockChartsContainer.deleteDashboard(id)
