...
def add_regions(apps, schema_editor):
    ...
    locations_to_increment = LocationType.objects.filter(
        admin_level__gte=1).order_by('-admin_level')
    ...
    for region in regions_dict.keys():
        random_location_code = ''.join(random.choice(
...
def run_agg(apps, schema_editor):
    ...
    # ensure that aggregation works by running the agg refresh in the
    # migration itself.
