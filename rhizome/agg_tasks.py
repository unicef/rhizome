...
class AggRefresh(object):
    ...
    def __init__(self, campaign_id=None):
        ...
        latest_dp_source = DataPoint.objects.filter(
            campaign_id=self.campaign.id).order_by('-created_at')[0].source_submission_id
    ...
    def build_calc_df(self, calc_list):
        ...
        calc_df = DataFrame(list(CalculatedIndicatorComponent.objects
                                 ...
                                 .values_list('indicator_id', 'indicator_component_id', 'calculation')), columns=['calc_indicator_id', 'indicator_component_id', 'calc'])
    ...
    def sum_of_parts(self):
        ...
        self_join_calc_df = calc_df.merge(
            calc_df, left_on='indicator_component_id', right_on='calc_indicator_id', how='left')
