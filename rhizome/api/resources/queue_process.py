...
from rhizome.models import SourceSubmission, DocumentDetail
    ...
    **GET Request:** Sets SourceSubmission objects for a given document id to "to process"
    ...
    def get_object_list(self, request):
        ...
        SourceSubmission.objects.filter(
            document_id=document_id).update(process_status='TO_PROCESS')

        queryset = DocumentDetail.objects \
            .filter(document_id=document_id).values('id', 'doc_detail_type_id', 'doc_detail_type__name', 'document_id', 'doc_detail_value')
