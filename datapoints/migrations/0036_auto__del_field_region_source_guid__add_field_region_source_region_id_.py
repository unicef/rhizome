...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Region', fields ['source', 'source_guid']
        db.delete_unique('region', ['source_id', 'source_guid'])

        # Deleting field 'Region.source_guid'
        db.delete_column('region', 'source_guid')

        # Adding field 'Region.source_region_id'
        db.add_column('region', 'source_region_id',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=484, to=orm['source_data.SourceRegion']),
                      keep_default=False)


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Region.source_guid'
        raise RuntimeError("Cannot reverse this migration. 'Region.source_guid' and its values cannot be restored.")

        # The following code is provided here to aid in writing a correct migration        # Adding field 'Region.source_guid'
        db.add_column('region', 'source_guid',
                      self.gf('django.db.models.fields.CharField')(max_length=255),
                      keep_default=False)

        # Deleting field 'Region.source_region_id'
        db.delete_column('region', 'source_region_id_id')

        # Adding unique constraint on 'Region', fields ['source', 'source_guid']
        db.create_unique('region', ['source_id', 'source_guid'])
    ...
    models = {
        ...
        u'datapoints.region': {
            ...
            'source_region_id': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['source_data.SourceRegion']"})
        },
