...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Region.full_name'
        db.delete_column('region', 'full_name')

        # Adding unique constraint on 'Region', fields ['name']
        db.create_unique('region', ['name'])


    def backwards(self, orm):
        # Removing unique constraint on 'Region', fields ['name']
        db.delete_unique('region', ['name'])


        # User chose to not deal with backwards NULL issues for 'Region.full_name'
        raise RuntimeError("Cannot reverse this migration. 'Region.full_name' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'Region.full_name'
        db.add_column('region', 'full_name',
                      self.gf('django.db.models.fields.CharField')(max_length=55, unique=True),
                      keep_default=False)
