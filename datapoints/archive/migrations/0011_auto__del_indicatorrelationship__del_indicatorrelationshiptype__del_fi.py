...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_column('region', 'description')

        # Deleting field 'Region.short_name'
        db.delete_column('region', 'short_name')

        # Adding unique constraint on 'Region', fields ['full_name']
        db.create_unique('region', ['full_name'])

        # Adding unique constraint on 'Indicator', fields ['name']
        db.create_unique('indicator', ['name'])

        # Adding unique constraint on 'Indicator', fields ['slug']
        db.create_unique('indicator', ['slug'])


    def backwards(self, orm):
        # Removing unique constraint on 'Indicator', fields ['slug']
        db.delete_unique('indicator', ['slug'])

        # Removing unique constraint on 'Indicator', fields ['name']
        db.delete_unique('indicator', ['name'])

        # Removing unique constraint on 'Region', fields ['full_name']
        db.delete_unique('region', ['full_name'])

        # Adding field 'Region.description'
        db.add_column('region', 'description',
                      self.gf('django.db.models.fields.CharField')(max_length=255, null=True),
                      keep_default=False)


        # User chose to not deal with backwards NULL issues for 'Region.short_name'
        raise RuntimeError("Cannot reverse this migration. 'Region.short_name' and its values cannot be restored.")

        # The following code is provided here to aid in writing a correct migration        # Adding field 'Region.short_name'
        db.add_column('region', 'short_name',
    ...
    models = {
        ...
        u'datapoints.region': {
            ...
            'slug': ('autoslug.fields.AutoSlugField', [], {'unique_with': '()', 'max_length': '50', 'populate_from': "'short_name'"})
        },
